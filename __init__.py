...
def import_mat(material_type, my_tiles_dir):
    ...
    if material_type == 'DEFAULT':
        ...
        if bpy.context.scene.render.engine == 'CYCLES':
            ...
            material_name = "maze_default_cycles"
...
# UI Classes
# 3D View
class MazeGeneratorPanel_MG(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene.use_list_maze:
            box.prop_search(scene, 'list_maze', bpy.data, 'texts', text = 
                "List Maze")        
...
class ImageConverterPanel_MG(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        row = box.row()
        row.label("(X: " + str(scene.mg_width) + ", Y: " + 
            str(scene.mg_height) + ")")
...
class MazeTilesPanel_MG(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene.tile_based:
            ...
            sub_box = box.box()
            sub_box.prop(context.scene, 'merge_objects', text = 
                "Merge Objects")
            if scene.merge_objects:
                sub_box.prop(context.scene, 'apply_modifiers', text = 
                    "Apply Modifiers")
                sub_box.prop(context.scene, 'remove_doubles_merge', text = 
                    "Remove Doubles")
            else:
            ...
            col.prop_search(context.scene, 'wall_4_sided', bpy.data, 
                "objects", "4 Sided Wall")
            col.prop_search(context.scene, 'wall_3_sided', bpy.data, 
                "objects", "3 Sided Wall")
            col.prop_search(context.scene, 'wall_2_sided', bpy.data, 
                "objects", "2 Sided Wall")
            col.prop_search(context.scene, 'wall_1_sided', bpy.data, 
                "objects", "1 Sided Wall")
            col.prop_search(context.scene, 'wall_0_sided', bpy.data, 
                "objects", "0 Sided Wall")
            col.prop_search(context.scene, 'wall_corner', bpy.data, 
                "objects", "Wall Corner")
            
            col = box.column()
            col.label("Floor Pieces:")
            col.prop_search(context.scene, 'floor_4_sided', bpy.data, 
                "objects", "4 Sided Floor")
            col.prop_search(context.scene, 'floor_3_sided', bpy.data, 
                "objects", "3 Sided Floor")
            col.prop_search(context.scene, 'floor_2_sided', bpy.data, 
                "objects", "2 Sided Floor")
            col.prop_search(context.scene, 'floor_1_sided', bpy.data, 
                "objects", "1 Sided Floor")
            col.prop_search(context.scene, 'floor_0_sided', bpy.data, 
                "objects", "0 Sided Floor")
            col.prop_search(context.scene, 'floor_corner', bpy.data, 
                "objects", "Floor Corner")


class BatchGeneratorPanel_MG(bpy.types.Panel):
...
class InfoPanel_MG(bpy.types.Panel):
    bl_label = "Info"
...
class HelpPanel_MG(bpy.types.Panel):
    bl_label = "Help"
    ...
    def draw(self, context):
        ...
        box = layout.box()
        if (scene.generation_desire == "SIMP_3D" and 
            scene.user_provision == "SETTINGS"):
        ...
        elif (scene.generation_desire == "SIMP_3D" and 
            scene.user_provision == "IMAGE_MAZE"):
            ...
            scene.user_provision == "TEXT_MAZE"):
        ...
        elif (scene.generation_desire == "TILE_MAZE" and 
            scene.user_provision == "SETTINGS"):
        ...
        elif (scene.generation_desire == "TEXT_MAZE" and 
            scene.user_provision == "SETTINGS"):
        ...
        elif (scene.generation_desire == "IMAGE_MAZE" and 
            scene.user_provision == "SETTINGS"):
...
class MazeAddonPrefs_mg(bpy.types.AddonPreferences):
    bl_idname = __name__
    ...
    def draw(self, context):
        ...
        if self.show_quickhelp:
            ...
            row.label(text="Blender freezes its UI when scripts are " +
                "executing, however, this can be combatted with a few " +
                "tricks as follows")
        ...
        row.scale_y = 1
        # website url
        row.operator("wm.url_open", text="Support Website", 
            icon='QUESTION').url = "http://integrity-sg.com"
        # e-mail
        row.operator("wm.url_open", text="Support E-Mail", 
            icon='LINENUMBERS_ON').url = ("mailto: assetsupp" + 
            "ort@integrity-sg.com")


# Text Editor
class MazeGeneratorTextToolsPanel_MG(bpy.types.Panel):
...
class ShowHelpDiagram_MG(bpy.types.Operator):
...
class ShowReadme_MG(bpy.types.Operator):
    bl_label = "Readme"
...
class DemoTiles_MG(bpy.types.Operator):
...
class GenerateMaze_MG(bpy.types.Operator):
...
# classes to register
classes = [GenerateMaze_MG, batch_gen.BatchGenerateMaze_MG, 
    batch_gen.StoreBatchMaze_MG, batch_gen.ClearBatchMazes_MG, 
    batch_gen.RefreshBatchMazes_MG, batch_gen.LoadBatchMaze_MG, 
    batch_gen.DeleteBatchMaze_MG, time_log.EstimateTime_MG, 
    MazeGeneratorPanel_MG, ImageConverterPanel_MG, MazeTilesPanel_MG, 
    BatchGeneratorPanel_MG, InfoPanel_MG, HelpPanel_MG, DemoTiles_MG, 
    MazeGeneratorTextToolsPanel_MG, text_tools.ReplaceText_MG, 
    text_tools.InvertText_MG, txt_img_converter.ConvertMazeImage_MG, 
    txt_img_converter.CreateImageFromList_MG, ShowHelpDiagram_MG, 
    ShowReadme_MG, MazeAddonPrefs_mg]
